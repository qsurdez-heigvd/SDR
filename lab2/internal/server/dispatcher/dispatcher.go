...
type dispatcherImpl struct {
	...
	sendChan chan<- messages.Destined[messages.Message]
...
func newDispatcher(logger *logging.Logger, selfAddr transport.Address, directNeighbors []transport.Address, network transport.NetworkInterface) *dispatcherImpl {
	sendChan := utils.NewBufferedChan[messages.Destined[messages.Message]]()
	...
	go func() {
		for msg := range sendChan.Outlet() {
			...
			if err != nil {
				d.logger.Errorf("Error sending message to %v: %v", msg.To, err)
			}
		}
	}()
...
func (d *dispatcherImpl) Send(msg Message, dest transport.Address) {
	d.sendChan <- messages.Destined[messages.Message]{
		To:      dest,
		Message: msg,
	}
}
