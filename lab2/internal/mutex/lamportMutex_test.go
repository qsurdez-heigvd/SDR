...
func TestRecvReq(t *testing.T) {
	// When a process that is not in CS nor requesting it receives a REQ, it should reply ACK, to the requester only.

	mockNetwork := newMockMutexNetwork("A", []Pid{"B", "C"})
	defer mockNetwork.close()
	NewLamportMutex(newLogger(), mockNetwork.asNetWrapper(), "A", []Pid{"B", "C"})
...
func TestRecvReqWhileInCS(t *testing.T) {
	// When a process that is in CS receives a REQ, it should reply ACK to the requester, before sending the REL upon exiting CS.
	...
	mockNetwork.SimulateReception(MockMessage{From: "B", To: "A", Message: Message{Type: ackMsg, TS: timestamp{Seqnum: 2, Pid: "B"}}})
	mockNetwork.SimulateReception(MockMessage{From: "C", To: "A", Message: Message{Type: ackMsg, TS: timestamp{Seqnum: 2, Pid: "C"}}})

	mockNetwork.SimulateReception(MockMessage{From: "B", To: "A", Message: Message{Type: reqMsg, TS: timestamp{Seqnum: 2, Pid: "B"}}})

	expectMessages(t, mockNetwork, []MockMessage{
		{From: "A", To: "B", Message: Message{Type: ackMsg, TS: timestamp{Seqnum: 5, Pid: "A"}}},
	}, 5*time.Second)

	close(mayRelease)

	expectMessages(t, mockNetwork, []MockMessage{
		{From: "A", To: "B", Message: Message{Type: relMsg, TS: timestamp{Seqnum: 6, Pid: "A"}}},
		{From: "A", To: "C", Message: Message{Type: relMsg, TS: timestamp{Seqnum: 6, Pid: "A"}}},
	}, 5*time.Second)
}
