...
type clientsManager struct {
	...
	broadcastReqs *utils.BufferedChan[common.ClientChatMessage]
...
Note that communication with the clients does not go through a dispatcher, but through the network interface directly, for simplicity.
*/
func NewClientsManager(logger *logging.Logger, self transport.Address, elector election.Elector, net transport.NetworkInterface) ClientsManager {
	m := clientsManager{
		...
		broadcastReqs:    utils.NewBufferedChan[common.ClientChatMessage](),
...
func (m *clientsManager) handleClients() {
	...
	for {
		...
		case msg := <-m.broadcastReqs.Outlet():
