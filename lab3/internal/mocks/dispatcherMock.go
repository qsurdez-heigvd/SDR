...
// Send sends a message to a specific destination.
func (d MockDispatcher) Send(msg message, dest address) {
	d.sentMessages <- destinedMessage{Message: msg, To: dest}
}

// SendAsync sends a message to a specific destination asynchronously.
func (d MockDispatcher) SendAsync(msg message, dest address) (result chan error) {
	d.Send(msg, dest)
